...
static inline int32_t getAccessMask(const DoorReader* doorReader, uint8_t mode, const char* doorname, sqlite3 *db) {
	return getAccessMask2(doorReader->pin, doorReader->pinLen, doorReader->card, doorReader->cardLen, mode, doorname, db);
}

static inline int32_t checkKeyAccess(const DoorReader* doorReader, sqlite3 *db) {
